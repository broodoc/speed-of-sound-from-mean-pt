...
void runAnalysis( bool twoPionOnly = true, 
		  const char* setting = "_200p0_COL", 
                  const char* fileList = "filesPico_200p0_2011.list",
  ...
  // SoundVelocity Maker
  StSoundVelocity *anaMaker = new StSoundVelocity( setting, pico ); // <- // StMultiPionFempto referenced here
  // Input root files
  ...
  EventLoop: if (iev<=nevents && istat!=2) {
    if(iev%100000==0) cout << "Working on eventNumber " << iev << endl;
    chain->Clear();
