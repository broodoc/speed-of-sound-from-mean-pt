...
void PlotTherm_r3(){
  ...
  TFile *infile=new TFile("Therm_FSI_b5.root","READ");
  ...
  */
  ...
  for(int i=1; i<=50; i++){
	...
	/*double num = r3num->GetBinContent(i,j,k)/r3numEn->GetBinContent(i,j,k);
	double den1 = r3den1->GetBinContent(i,j,k)/r3den1En->GetBinContent(i,j,k);
	double den2 = r3den2->GetBinContent(i,j,k)/r3den2En->GetBinContent(i,j,k);
	double den3 = r3den3->GetBinContent(i,j,k)/r3den3En->GetBinContent(i,j,k);*/
	...
	double den1 = R2->GetBinContent(i);
	double den2 = R2->GetBinContent(j);
	double den3 = R2->GetBinContent(k);
	if(den1*den2*den3 <= 0) continue;
	
	double den = sqrt(den1*den2*den3);
	double weightWidth = sqrt( fabs(r3den1Sq->GetBinContent(i,j,k) - pow(r3den1->GetBinContent(i,j,k),2))/r3den1En->GetBinContent(i,j,k));
	...
	r3->Fill(q3,num);
	//r3den->Fill(q3,den);
	r3den->Fill(q3,den * r3numEn->GetBinContent(i,j,k));
	//
	r3num_e[int(q3/0.01)] += (r3numSq->GetBinContent(i,j,k)-pow(r3num->GetBinContent(i,j,k),2))/r3numEn->GetBinContent(i,j,k);
  ...
  for(int i=0; i<10; i++){
    ...
    r3den->SetBinError(i+1,sqrt(r3den_e[i]));
  }
  //r3->Sumw2();
  ...
  r3->GetXaxis()->SetTitleOffset(0.9);
  r3->GetYaxis()->SetTitleSize(0.05);
  r3->GetYaxis()->SetTitleOffset(0.9);
  r3->GetXaxis()->SetTitle("Q_{3} (GeV/c)");
  r3->GetYaxis()->SetTitle("r_{3}");
  r3->SetTitle("");
  r3->GetXaxis()->SetRangeUser(0,0.09);
  r3->Draw();
  ...
  TLatex *Specif = new TLatex(0.005,.4,"Therminator");// Therminator
