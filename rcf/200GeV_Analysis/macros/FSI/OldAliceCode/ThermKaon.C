...
#define PI 3.1415926
#define BohrR 562.51 // 111/0.197327 (562.51)
...
{// b is assumed to be 1.0 here
  ...
  if(abs(z) > 20.) {// high k*r* case (20., 30. for strict case)
    double eta = -a.imag();
    double RKS = z.imag();
...
void ThermKaon(){
  ...
  bool StrongFSI=kFALSE;

  massPOI = massK0;// masspiC, massKch, massK0
  PIDPOI = 311;// 211 for pi+-, 321 for K+-, 311 for K0
  qCutOff = 0.2;// 0.1 for pi+-, 0.2 for others

  int Nfiles=500;// 1000 for b2, 500 for rest
  int bValue=5;

  TFile *outfile = new TFile("Therm_dist_tempKaon.root","RECREATE");
  ...
  TH2D *NumSq_Cos_ss=new TH2D("NumSq_Cos_ss","",20,0,1, 40,0,0.2);
  TH2D *NumSq_Cos_os=new TH2D("NumSq_Cos_os","",20,0,1, 40,0,0.2);
  ...
  //
  //
  TH1D *Ptspectrum = new TH1D("Ptspectrum","",100,0,10);
  TProfile *v2vsPt = new TProfile("v2vsPt","",100,0,10, -1,1,"");
  ...
  for(int FileN=0; FileN<Nfiles; FileN++){
    ...
    *fname += FileN+1;
	...
	if(t < 1e6) Primary[0][Npions]=kTRUE;
	else {Primary[0][Npions]=kFALSE;}
	
	if(Primary[0][Npions]) {
	  float phi = atan2(py,px);
	  Ptspectrum->Fill(pt);
	  v2vsPt->Fill(pt, cos(2*phi));
	}
	...
	for(int index1=0; index1<Nparticles[EN]; index1++){// current or past event pion loop
	  ...
	  for(int index2=start; index2<Npions; index2++){// current event pion loop (EN=0)
	    ...
	    if(qinv12 < 0.2 && rstar12_PRF < RstarMax){
	    ...
	    if(PID[EN][index1] == PID[0][index2]){// same-charge
	      ...
	      NumSq_Cos_ss->Fill(kt, qinv12, pow(BEE,2));
	      ...
	      NumSq_Cos_os->Fill(kt, qinv12, pow(BEE,2));
