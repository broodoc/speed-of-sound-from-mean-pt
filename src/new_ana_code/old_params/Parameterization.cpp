...
double Parameterization::operator()(double *x, double *par) {
    ...
    switch (type_f) {
        ...
        case Function::Hagedorn:
            ...
            // b - par[2]
            // constant - par[3]
            return (xx * xx / sqrt(par[0] * par[0] + xx * xx)) *
            ...
            // n - par[2]
            // T - par[3]
//            return xx * pow((1 + (1/sqrt(1 - pow(par[1],2))) 
//                             * ((sqrt(pow(xx,2))
//                            + pow(par[0],2)) - (xx * par[1])
//                        / (par[2] * par[3]) )), -par[2]);
//            return xx * pow((1 + (1/sqrt(1 - pow(par[1],2)))
//                             * ((sqrt(pow(xx,2))
//                            + pow(par[0],2)) - (xx * par[1])
...
TF1* Parameterization::GetFit(TH1* h, double low_cut, double high_cut, Parameterization::Function fitFuncType, const TString& name) {
...
//    std::cout << "Selecting fit" << std::endl;
    // Set default parameters (customize per function)
    switch (fitFuncType) {
        case Function::LevyTsallis:
//            fit->SetParameters(0.1, 0.15, 12, 10000);
//            fit->SetParameters(0.13957, 0.08, 8., 5500); //19.6 GeV
//            fit->SetParameters(5.741e-06, 0.1528, 7.8, 5500); //200 GeV
//            fit->SetParameters(0.139, 0.08, 7.8, 5500); //19.6 GeV
            fit->SetParameters(0.139, 0.158, 9.022, 5500); 
//            fit->SetParameters(0.08, 0.07, 7.9, 1000);
            //27GeV
//            fit->FixParameter(0, 0.13957); //pion mass for inclusive spectra
//            fit->SetParLimits(0, 5.741e-06, 5.75e-06);  // m
//            fit->SetParLimits(1, 0.15, 0.16);  // C
//            fit->SetParLimits(2, 7.81, 7.9);  // n
            
//            fit->FixParameter(0, 0.139);
//            fit->SetParLimits(0, 1.074e-6, 0.2);
//            fit->FixParameter(1, 0.1515);
//            fit->SetParLimits(1, 0.156,0.159);
//            fit->FixParameter(2, 6.998);
//            fit->SetParLimits(2, 7., 10.);
//            fit->SetParLimits(3, 3000, 9000);   // dN/dy
        ...
        case Function::Tsallis:
            ...
            break;
        case Function::Hagedorn:
            fit->SetParameters(0.139, 0.2, 6.5, 5000);
            
//            fit->FixParameter(0, -5.281e-6);
//            fit->FixParameter(1, 1.427);
////            fit->SetParLimits(1, 0.1,1.0);
//            fit->FixParameter(2, 9.022);
////            fit->SetParLimits(2, 6.,7.);
//            fit->SetParLimits(3, 3000, 1.6e4);   // dN/dy
            
            fit->SetParNames("m", "beta_T", "M", "T");
...
//                fit->SetParameters(1.6e4, 3e4, 5.0);
                fit->SetParameters(1.616e4, 3e4, 6.011);
            fit->SetParLimits(0, 0, 1e6);
            fit->SetParLimits(1, 0, 1e6);
            fit->SetParLimits(2, 1.0, 10.0);

//                fit->FixParameter(0, 1.424e4);
//                fit->FixParameter(1, 2.375e4);
//                fit->FixParameter(2, 5.931);
//                fit->SetParLimits(0, 1.6e4, 1.75e4);
//                fit->SetParLimits(1, 2.3e4, 2.9e4);
//                fit->SetParLimits(2, 5.5, 7.5);
...
//            fit->FixParameters(1.0, TOTAL_CHARGED_MASS, 0.114, 1.086, 0.);
            fit->SetParameters(0, 500000.0);  // A is a free parameter
            fit->FixParameter(1, TOTAL_CHARGED_MASS); //m
//            fit->SetParameters(1, TOTAL_CHARGED_MASS); //m
//            fit->FixParameter(2, 0.114); //T
            fit->FixParameter(2, 0.07); //T
//            fit->FixParameter(2, 0.1); //T
//            fit->SetParameters(2, 0.114); //T
//            fit->SetParLimits(2, 0.113, 0.115);
            fit->FixParameter(3, 1.086); //q
//            fit->FixParameter(3, 1.1); //q
//            fit->FixParameter(3, 1.05); //q
//            fit->SetParameters(3, 1.09); //q
//            fit->SetParLimits(3, 1.08, 1.1); //q
//            fit->FixParameter(4, 0.0);   //beta_s
            fit->SetParameters(4, 0.0);   //beta_s
//            fit->SetParameters(4, 0.743);   //beta_s
...
//    std::cout << "Applying fit" << std::endl;
//    TFitResultPtr fitResult = h->Fit(fit, "SMQR");
    TFitResultPtr fitResult = h->Fit(fit, "SQR");
//    TFitResultPtr fitResult = h->Fit(fit, "SQR");
//    TFitResultPtr fitResult = h->Fit(fit, "QR0");
//    if (!fitResult->IsValid()) {
//        std::cerr << "WARNING: Fit failed with function "
//                  << fitname << std::endl;
//        return fit;
//    }
...
int Parameterization::GetNParams(Parameterization::Function f) {
    switch (f) {
        ...
        case Parameterization::Function::Hagedorn:          return 4;
...
//    std::cout << "\r\033[2K";
//    std::cout << "\rPerforming c_s^2 fit for "
//              << low_cut << " -> " << high_cut
//              << "(pT ∈ [" << pTlow << "," << pThigh << "])...🧮"
//              << std::flush;
    TF1 *PowerLaw_fit = new TF1(fitName,Parameterization::PowerLaw,low_cut, high_cut,2); // 2 parameters (cs2 = par0, normalization = par1)
//    TF1 *Right_knee_fit = new TF1(fitName,Parameterization::RightKneeFunction,low_cut, high_cut,2); // 2 parameters (cs2 = par1, normalization = par0)
    
    PowerLaw_fit->SetParameter(0, 0.2);     // cs2
    PowerLaw_fit->SetParameter(1, 1.05);    // normalization (in accordance with knee)
    ...
    PowerLaw_fit->SetLineWidth(4.0);
    if (debug) std::cout << "Placeholder" << std::endl;
    TFitResultPtr fitResult = nullptr;
//    TFitResultPtr fitResult = h_fit->Fit(PowerLaw_fit, "SRQ");  // "S" ensures the fit result is returned
//    TFitResultPtr fitResult = h_fit->Fit(Right_knee_fit, "SRQ");  // "S" ensures the fit result is returned
    try {
        fitResult = h_fit->Fit(PowerLaw_fit, "SMREQ");
    } catch (...) {
...
//    std::vector<Function> fitFunctions = {Function::LevyTsallis, Function::Hagedorn, Function::FokkerPlanck, Function::Tsallis, Function::ExpDecay, Function::TBW, Function:BW};
    std::vector<Function> fitFunctions = {Function::LevyTsallis, Function::Hagedorn, Function::ExpDecay};
...
//            int ref_bin{13}; // <- you can try varying reference class here 5%->13 (OLD: 5%->17; 10%->23)
//            int ref_bin{12}; // <- you can try varying reference class here 5%->12 (OLD: 5%->17; 10%->23)
            int ref_bin{12}; // <- you can try varying reference class here 5%->12 (OLD: 5%->17; 10%->23)
            //        int ref_bin{16}; //9 -> 8, 17 -> 16, 18 -> 17
...
//                    std::cout << "Writing reference spectrum: " << hpt_ref->GetName() << std::endl;
                ...
                //        const double dndetaAna_ref{params.GetdNdEta(hpt_ref,fit,true,10,0.2)};
...
//                    if (k < 20 && (chi2ndf > 100. || chi2ndf < 1e-9)) continue;
//                    if (k < 20 && (chi2ndf > 1000. || chi2ndf < 1e-9)) continue;
//                    if (k < 20 && (chi2ndf > 10000. || chi2ndf < 1e-9)) continue;
                    fitting_score += params.ComputeFitScore(chi2ndf);
...
//                    error_dndeta_ratio = error_dndeta_ratio + 0.008*error_dndeta_ratio; //sys uncertainty on dNdeta ratio
                    ...
                    //            gdN->GetYaxis()->SetRangeUser(0.99, 1.01);
                    gdN->GetYaxis()->SetRangeUser(0.99, 1.025);
                    //            gdN->GetXaxis()->SetRangeUser(0.8, 1.25);
                    //            gdN->GetYaxis()->SetRangeUser(0.98, 1.025);
