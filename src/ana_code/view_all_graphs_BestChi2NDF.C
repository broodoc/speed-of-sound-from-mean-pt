    ...
    if (obj->InheritsFrom("TGraph")) {
        ...
        for (int i = 0; i < nFrames; ++i) {
            if (i == (nFrames - 1)){
                ...
                g->GetXaxis()->SetRangeUser(x_mins[i], 1.15);
                g->GetYaxis()->SetRangeUser(y_mins[i], 1.025);
                c->cd();
                g->Draw("P E");
                c->Update();
            ...
            graph->GetXaxis()->SetRangeUser(x_mins[i], 1.15);
            graph->GetYaxis()->SetRangeUser(y_mins[i], 1.025);
        ...
        for (int i = 0; i < nFrames; ++i) {
            hist->GetXaxis()->SetRangeUser(x_mins[i], 1.15);
            hist->GetYaxis()->SetRangeUser(y_mins[i], 1.025);
...
//                continue;
                ...
                continue;
                format_ptstudy_norm(c, fetched, graphName, outputDirPath);
            }
...
//                    }
            ...
            if (rawName.Contains("spectra_study")) {
                ...
                format_ptstudy_norm(c, obj, graphName, outputDirPath);
            } else if (rawName.Contains("Absolute")) {
                format_ptstudy_absolute(c, obj, graphName, outputDirPath);
            }
...
void view_all_graphs_BestChi2NDF(const char* rootFilePath, const char* outputDirPath) {
    ...
    TDirectory* bestDir = (TDirectory*)file->Get("Best Chi2Ndf on spectra");
    if (!bestDir) {
        std::cerr << "Error: 'Best Chi2Ndf on spectra' directory not found.\n";
        return;
    }

    processDirectory(bestDir, outputDirPath, "BestChi2Ndf");
    file->Close();
}
